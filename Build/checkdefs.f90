...
!
     ...
     &   'Analytical kinematic bottom temperature flux.'
      ...
      IF (Master) WRITE (stdout,20) 'ANA_ICEDRAG_GRID',                 &
     &   'Analytical spatially varying quadratic ice-ocean drag coeff.'
      is=LEN_TRIM(Coptions)+1
      Coptions(is:is+13)=' ANA_ICEDRAG,'
     ...
     &   'Fourth-order centered vertical advection of momentum.'
      ...
      ivelVadv=ivelVadv+1
      IF (Master) WRITE (stdout,20) 'UV_ICEDRAG_GRID',                  &
     &   'Spatially varying quadratic drag coefficient.'
      is=LEN_TRIM(Coptions)+1
      Coptions(is:is+17)=' UV_ICEDRAG_GRID,'
